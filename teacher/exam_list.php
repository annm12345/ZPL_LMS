...
if(isset($_POST['create_exam'])){
    ...
    $paper = $_POST['exam_paper'];
    $date = $_POST['exam_date'];
    date_default_timezone_set('Asia/Yangon');
    $added_on = date('Y-m-d h:i:s');
    mysqli_query($con, "INSERT INTO `exam_list`(`name`, `course`,`email`, `date`, `added_on`) VALUES ('$name','$paper','$email','$date','$added_on')");
    echo '<script>window.alert("New exam created successfully!");</script>';
}
}
...
if(isset($_GET['id']) && isset($_GET['action'])){
    ...
    if($action=='hide'){
        mysqli_query($con,"UPDATE `exam_list` SET `action`='1' WHERE `id`='$id'");
    }elseif($action=='unhide'){
        mysqli_query($con,"UPDATE `exam_list` SET `action`='0' WHERE `id`='$id'");
    }elseif($action=='delete'){
        mysqli_query($con,"DELETE FROM `exam_list` WHERE `id`='$id'");
    }
}
        ...
        <div class="modal-content-list" >
            ...
            <table id="exam-table">
                ...
                <?php
                    if(isset($_SESSION['TEACHER_LOGIN'])){
                    $email = $_SESSION['TEACHER_EMAIL'];
                    $res = mysqli_query($con, "SELECT * FROM `exam_list` WHERE email='$email' order by `date` ASC");
                ...
                <tr>
                    ...
                    <th><a href="question_list.php?id=<?php echo $row['id'] ?>"><i class="fa-solid fa-eye" style="color:green;padding:0.5rem;font-size:1rem;"></i></a></th>                   
                    <td><a href=""><a href="exam_list.php?id=<?php echo $row['id'] ?>&action=delete"> <i class="fa-solid fa-trash" style="color:red;padding:0.5rem;font-size:1rem;"></i> </a></td>
                    <td><a href="exam_result.php?id=<?php echo $row['id'] ?>" style="background: blue; color: #fff; padding: 0.5rem 1rem; border-radius: 10px; display: inline-block; text-decoration: none;">Exam result</a>

                    <?php
                    if($row['action']=='0'){
                    ?>
                    <a href="exam_list.php?id=<?php echo $row['id'] ?>&action=hide" style="background: red; color: #fff; padding: 0.5rem 1rem; border-radius: 10px; display: inline-block; text-decoration: none;">Hide result</a></td>
                    <?php }else{
                        ?>
                            <a href="exam_list.php?id=<?php echo $row['id'] ?>&action=unhide" style="background: green; color: #fff; padding: 0.5rem 1rem; border-radius: 10px; display: inline-block; text-decoration: none;">Unhide result</a></td>
                        <?php
                    } ?>
                </tr>
                <?php
